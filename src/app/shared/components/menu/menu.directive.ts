...
import { booleanAttribute, computed, Directive, effect, ElementRef, inject, input, OnDestroy, OnInit, untracked } from '@angular/core';

import { WINDOW, NAVIGATOR, DOCUMENT_TOKEN } from '../../../core/tokens/browser.tokens';
...
export class ZardMenuDirective implements OnInit, OnDestroy {
  ...
  private setupMenuContentListeners(): void {
    ...
    const overlay = this.document.querySelector(ZardMenuDirective.MENU_OVERLAY_SELECTOR);
    if (!overlay) return;
  ...
  private isMobileDevice(): boolean {
    if (!this.window || !this.navigator) {
      return false; // Default to desktop behavior on server
    }

    // Check for touch support
    const hasTouch = 'ontouchstart' in this.window || this.navigator.maxTouchPoints > 0;

    // Check for mobile user agent
    const mobileRegex = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i;
    const isMobileUA = mobileRegex.test(this.navigator.userAgent);

    // Check viewport width for small screens
    const isSmallScreen = this.window.innerWidth <= 768;
